...
async def print_occupancy_grid() -> None:
    ...
    cmap = colors.ListedColormap(
        [UNSPECIFIED, FREE_COLOUR, OCCLUDED_COLOUR, STATIC_OCCUPIED_COLOUR]
    )
    ...
    def print_grid(event: horus.OccupancyGridEvent) -> None:
        ...
        legend_labels = ["Free", "Occluded", "Stationary Occupied"]
        legend_colors = [FREE_COLOUR, OCCLUDED_COLOUR, STATIC_OCCUPIED_COLOUR]
        legend_patches = [
            patches.Patch(color=color, label=label)
            for color, label in zip(legend_colors, legend_labels)
        ]
        plt.legend(handles=legend_patches, loc="upper right", title="Classification")
        plt.imshow(grid_map, cmap=cmap, vmin=0, vmax=3)
        plt.pause(0.01)
