...
struct HasStringifySize<
    T,
    std::enable_if_t<
        std::is_same<decltype(HorusStringifySize(std::declval<T const&>())), std::size_t>::value>>
    : std::integral_constant<bool, noexcept(HorusStringifySize(std::declval<T const&>()))> {};
...
HORUS_NO_SANITIZE("unsigned-integer-overflow")
constexpr std::size_t
    ComputeExpectedSizeSum(std::size_t sum, T const& value, Rest const&... rest) noexcept {
...
class StringSink final {
 public:
  ...
  explicit StringSink(std::string& output HORUS_LIFETIME_BOUND) noexcept : output_{output} {}
...
template <class... Ts>
constexpr std::size_t StrExpectedSize(Ts const&... values) noexcept {
