...
// NOLINTBEGIN(*-identifier-naming): match `std::span<T>` naming.

/// A minimal span.
///
/// @see https://en.cppreference.com/w/cpp/container/span
template <class T>
class HORUS_VIEW_TYPE Span final {
 public:
  ...
  constexpr Span(T* begin HORUS_LIFETIME_BOUND, T* end HORUS_LIFETIME_BOUND) noexcept
      : begin_{begin}, end_{end} {
    assert(begin <= end);
  }

  /// Constructs a span starting at `begin` and of size `size`.
  constexpr Span(T* begin HORUS_LIFETIME_BOUND, std::size_t size) noexcept
      : begin_{begin}, end_{PointerAdd(begin, size)} {
  ...
  constexpr Span(std::array<std::remove_const_t<T>, N> const& array  // NOLINT(*-explicit-*)
                     HORUS_LIFETIME_BOUND) noexcept
      : Span{&std::get<0>(array), array.size()} {}
  ...
  constexpr std::size_t size() const noexcept { return PointerDist(begin_, end_); }
  ...
  constexpr Span subspan(std::size_t offset) const noexcept {
    assert(offset <= size());
    return Span{PointerAdd(begin_, offset), end_};
  }
