...
UnalignedSpan<T> CowSpan<T>::Span() const noexcept {
  HORUS_ONEOF_SWITCH(data_) {
    HORUS_ONEOF_CASE(vector, std::vector<T>) {
...
std::vector<T>& CowSpan<T>::Vector() noexcept(false) {
  ...
  HORUS_ONEOF_SWITCH(data_) {
    HORUS_ONEOF_CASE(vector, std::vector<T>) { return vector; }
    HORUS_ONEOF_CASE(view_ref, PbView) {
      const PbView view{std::move(view_ref)};
