...
class ProfilingSet final : public PbMessage {
 public:
  ...
  /// Returns whether `node_id` (no 5) is set.
  constexpr bool has_node_id() const noexcept { return set_fields_[3]; }
  ...
  /// Sets `node_id` (no 5) and returns `*this`.
  ProfilingSet& set_node_id(CowBytes&& node_id) & noexcept {
    set_fields_[3] = true;
...
///
/// Source: horus/pb/profiling.proto:78:1
class FrameProfiling final : public PbMessage {
 public:
  ...
  // Field `preprocessing_overhead` (no 4).
  // -----

  /// No documentation.
  ///
  /// Field no: 4.
  constexpr const Duration& preprocessing_overhead() const& noexcept HORUS_LIFETIME_BOUND {
    return preprocessing_overhead_;
  }

  /// If `preprocessing_overhead` is set, moves it out of the message (without marking it as unset).
  ///
  /// Otherwise, returns a default-initialized value.
  ///
  /// Field no: 4.
  Duration preprocessing_overhead() && noexcept {
    if (!set_fields_[2]) {
      return {};
    }
    return std::move(preprocessing_overhead_);
  }

  /// No documentation.
  ///
  /// Field no: 4.
  Duration& mutable_preprocessing_overhead() & noexcept HORUS_LIFETIME_BOUND {
    set_fields_[2] = true;
    return preprocessing_overhead_;
  }

  /// Returns whether `preprocessing_overhead` (no 4) is set.
  constexpr bool has_preprocessing_overhead() const noexcept { return set_fields_[2]; }

  /// Clears `preprocessing_overhead` (no 4).
  void clear_preprocessing_overhead() & noexcept {
    set_fields_[2] = false;
    preprocessing_overhead_ = {};
  }

  /// Sets `preprocessing_overhead` (no 4) and returns `*this`.
  FrameProfiling& set_preprocessing_overhead(Duration&& preprocessing_overhead) & noexcept {
    set_fields_[2] = true;
    preprocessing_overhead_ = std::move(preprocessing_overhead);
    return *this;
  }
  /// Sets `preprocessing_overhead` (no 4) and returns `*this`.
  FrameProfiling&& set_preprocessing_overhead(Duration&& preprocessing_overhead) && noexcept {
    return std::move(set_preprocessing_overhead(std::move(preprocessing_overhead)));
  }
 ...
 private:
  ...
  /// @see preprocessing_overhead()
  Duration preprocessing_overhead_{};
...
///
/// Source: horus/pb/profiling.proto:99:1
...
///
/// Source: horus/pb/profiling.proto:132:1
class DetectionMergerFrameProfiling final : public PbMessage {
 public:

  /// Constructs a default-initialized `DetectionMergerFrameProfiling`.
  DetectionMergerFrameProfiling() noexcept = default;

  /// Move constructor.
  DetectionMergerFrameProfiling(DetectionMergerFrameProfiling&&) noexcept = default;
  /// Move assignment operator.
  DetectionMergerFrameProfiling& operator=(DetectionMergerFrameProfiling&&) noexcept = default;

  /// Constructs a clone of `other`.
  ///
  /// @throws std::bad_alloc If `other` owns heap-allocated data which could not be cloned due to a
  /// lack of available memory.
  explicit DetectionMergerFrameProfiling(const DetectionMergerFrameProfiling& other) noexcept(false);  // NOLINT(*-explicit-*)

  /// Cannot copy-assign to avoid implicit allocations.
  DetectionMergerFrameProfiling& operator=(const DetectionMergerFrameProfiling&) = delete;

  /// Default destructor.
  ~DetectionMergerFrameProfiling() noexcept final = default;

  /// Creates a `DetectionMergerFrameProfiling` whose contents are read from `reader`.
  ///
  /// @throws InvalidProtobufMessage If the `reader` contains an invalid Protobuf message.
  explicit DetectionMergerFrameProfiling(PbReader& reader) noexcept(false) : PbMessage{} {
    DeserializeFrom(reader);
  }
  ...
  /// The full name of the message: `horus.pb.DetectionMergerFrameProfiling`.
  static constexpr StringView TypeName() noexcept { return "horus.pb.DetectionMergerFrameProfiling"; }

  /// The full name of the message: `horus.pb.DetectionMergerFrameProfiling`.
  StringView MessageTypeName() const noexcept final { return TypeName(); }

  // Field `detection_merger_overhead` (no 1).
  // -----
  ...
  ///
  /// Field no: 1.
  constexpr const Duration& detection_merger_overhead() const& noexcept HORUS_LIFETIME_BOUND {
    return detection_merger_overhead_;
  }

  /// If `detection_merger_overhead` is set, moves it out of the message (without marking it as unset).
  ///
  /// Otherwise, returns a default-initialized value.
  ///
  /// Field no: 1.
  Duration detection_merger_overhead() && noexcept {
    if (!set_fields_[0]) {
      return {};
    }
    return std::move(detection_merger_overhead_);
  }
  ...
  ///
  /// Field no: 1.
  Duration& mutable_detection_merger_overhead() & noexcept HORUS_LIFETIME_BOUND {
    set_fields_[0] = true;
    return detection_merger_overhead_;
  }

  /// Returns whether `detection_merger_overhead` (no 1) is set.
  constexpr bool has_detection_merger_overhead() const noexcept { return set_fields_[0]; }

  /// Clears `detection_merger_overhead` (no 1).
  void clear_detection_merger_overhead() & noexcept {
    set_fields_[0] = false;
    detection_merger_overhead_ = {};
  }

  /// Sets `detection_merger_overhead` (no 1) and returns `*this`.
  DetectionMergerFrameProfiling& set_detection_merger_overhead(Duration&& detection_merger_overhead) & noexcept {
    set_fields_[0] = true;
    detection_merger_overhead_ = std::move(detection_merger_overhead);
    return *this;
  }
  /// Sets `detection_merger_overhead` (no 1) and returns `*this`.
  DetectionMergerFrameProfiling&& set_detection_merger_overhead(Duration&& detection_merger_overhead) && noexcept {
    return std::move(set_detection_merger_overhead(std::move(detection_merger_overhead)));
  }

  // Field `total_overall_frame_latency` (no 2).
  // -----
  ...
  ///
  /// Field no: 2.
  constexpr const Duration& total_overall_frame_latency() const& noexcept HORUS_LIFETIME_BOUND {
    return total_overall_frame_latency_;
  }

  /// If `total_overall_frame_latency` is set, moves it out of the message (without marking it as unset).
  ///
  /// Otherwise, returns a default-initialized value.
  ///
  /// Field no: 2.
  Duration total_overall_frame_latency() && noexcept {
    if (!set_fields_[1]) {
      return {};
    }
    return std::move(total_overall_frame_latency_);
  }
  ...
  ///
  /// Field no: 2.
  Duration& mutable_total_overall_frame_latency() & noexcept HORUS_LIFETIME_BOUND {
    set_fields_[1] = true;
    return total_overall_frame_latency_;
  }

  /// Returns whether `total_overall_frame_latency` (no 2) is set.
  constexpr bool has_total_overall_frame_latency() const noexcept { return set_fields_[1]; }

  /// Clears `total_overall_frame_latency` (no 2).
  void clear_total_overall_frame_latency() & noexcept {
    set_fields_[1] = false;
    total_overall_frame_latency_ = {};
  }

  /// Sets `total_overall_frame_latency` (no 2) and returns `*this`.
  DetectionMergerFrameProfiling& set_total_overall_frame_latency(Duration&& total_overall_frame_latency) & noexcept {
    set_fields_[1] = true;
    total_overall_frame_latency_ = std::move(total_overall_frame_latency);
    return *this;
  }
  /// Sets `total_overall_frame_latency` (no 2) and returns `*this`.
  DetectionMergerFrameProfiling&& set_total_overall_frame_latency(Duration&& total_overall_frame_latency) && noexcept {
    return std::move(set_total_overall_frame_latency(std::move(total_overall_frame_latency)));
  }

 private:
  /// @see detection_merger_overhead()
  Duration detection_merger_overhead_{};
  /// @see total_overall_frame_latency()
  Duration total_overall_frame_latency_{};
...
/// / subscribers.
///
/// Source: horus/pb/profiling.proto:146:1
class ProfilingInfo final : public PbMessage {
 public:
  ...
  // Field `detection_merger_frame_profiling` (no 4).
  // -----

  /// / Per-merged-frame profiling produced by the detection merger service.
  ///
  /// Field no: 4.
  constexpr const DetectionMergerFrameProfiling& detection_merger_frame_profiling() const& noexcept HORUS_LIFETIME_BOUND {
    return detection_merger_frame_profiling_;
  }

  /// If `detection_merger_frame_profiling` is set, moves it out of the message (without marking it as unset).
  ///
  /// Otherwise, returns a default-initialized value.
  ///
  /// Field no: 4.
  DetectionMergerFrameProfiling detection_merger_frame_profiling() && noexcept {
    if (!set_fields_[3]) {
      return {};
    }
    return std::move(detection_merger_frame_profiling_);
  }

  /// / Per-merged-frame profiling produced by the detection merger service.
  ///
  /// Field no: 4.
  DetectionMergerFrameProfiling& mutable_detection_merger_frame_profiling() & noexcept HORUS_LIFETIME_BOUND {
    clear_profiling_set();
  ...
  /// Returns whether `detection_merger_frame_profiling` (no 4) is set.
  constexpr bool has_detection_merger_frame_profiling() const noexcept { return set_fields_[3]; }

  /// Clears `detection_merger_frame_profiling` (no 4).
  void clear_detection_merger_frame_profiling() & noexcept {
    profiling_set_ = {};
    set_fields_[3] = false;
    detection_merger_frame_profiling_ = {};
  }

  /// Sets `detection_merger_frame_profiling` (no 4) and returns `*this`.
  ProfilingInfo& set_detection_merger_frame_profiling(DetectionMergerFrameProfiling&& detection_merger_frame_profiling) & noexcept {
    clear_profiling_set();
    profiling_set_ = ProfilingSetOneof::kDetectionMergerFrameProfiling;
    set_fields_[3] = true;
    detection_merger_frame_profiling_ = std::move(detection_merger_frame_profiling);
    return *this;
  }
  /// Sets `detection_merger_frame_profiling` (no 4) and returns `*this`.
  ProfilingInfo&& set_detection_merger_frame_profiling(DetectionMergerFrameProfiling&& detection_merger_frame_profiling) && noexcept {
    return std::move(set_detection_merger_frame_profiling(std::move(detection_merger_frame_profiling)));
  }
  ...
  enum class ProfilingSetOneof : std::uint32_t {  // NOLINT(*-enum-size)
    ...
    /// @see detection_merger_frame_profiling()
    kDetectionMergerFrameProfiling = 4,
  };
  ...
  void clear_profiling_set() noexcept {
    switch (profiling_set_) {
      ...
      case ProfilingSetOneof::kDetectionMergerFrameProfiling: {
        clear_detection_merger_frame_profiling();
        break;
      }
 ...
 private:
  ...
  /// @see detection_merger_frame_profiling()
  DetectionMergerFrameProfiling detection_merger_frame_profiling_{};
